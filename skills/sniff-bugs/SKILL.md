...
3. **Trace** — Follow error returns, `defer`/`finally`, context cancellation, and goroutine/task spawn to completion.
4. **Report** — Findings table (see Output). Rank by severity. Cite `path:line` and the failure mode.

**quick:** categories 1–4 only. **thorough:** all categories plus language notes in [references/runtime.md](references/runtime.md).
